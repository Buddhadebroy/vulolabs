...
class FixHeadingHierarchyAction extends AbstractBasicAction {
    ...
    public function validate_input( array $input ): array {
        ...
        if ( ! $this->has_heading_level_skip( $post->post_content ) ) {
            throw new VuloPilotException( esc_html__( 'This post\'s heading levels do not skip - there is nothing to fix.', 'vulopilot' ), VuloPilotException::TYPE_INVALID_ACTION_INPUT );  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- false positive: flags the VuloPilotException::TYPE_* constant token itself, not unescaped output; the message argument is already esc_html()-wrapped.
        }
    ...
    public function validate_output( array $output, array $input ): void {
        ...
        if ( $this->has_heading_level_skip( $rewritten ) ) {
            throw new VuloPilotException( esc_html__( 'The AI did not resolve the heading level skip - rejected.', 'vulopilot' ), VuloPilotException::TYPE_INVALID_ACTION_OUTPUT );  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- false positive: flags the VuloPilotException::TYPE_* constant token itself, not unescaped output; the message argument is already esc_html()-wrapped.
        }
    }
