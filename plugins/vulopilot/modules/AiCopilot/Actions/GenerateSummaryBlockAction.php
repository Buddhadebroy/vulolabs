...
class GenerateSummaryBlockAction extends AbstractBasicAction {
    ...
    public function validate_output( array $output, array $input ): void {
        ...
        if ( empty( $takeaways ) || ! is_array( $takeaways ) ) {
            throw new VuloPilotException( esc_html__( 'The AI did not return any key takeaways.', 'vulopilot' ), VuloPilotException::TYPE_INVALID_ACTION_OUTPUT );  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- false positive: flags the VuloPilotException::TYPE_* constant token itself, not unescaped output; the message argument is already esc_html()-wrapped.
        }

        foreach ( $takeaways as $takeaway ) {
            if ( ! is_string( $takeaway ) || '' === trim( $takeaway ) ) {
                throw new VuloPilotException( esc_html__( 'The AI returned an empty key takeaway.', 'vulopilot' ), VuloPilotException::TYPE_INVALID_ACTION_OUTPUT );  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- false positive: flags the VuloPilotException::TYPE_* constant token itself, not unescaped output; the message argument is already esc_html()-wrapped.
            }
        }
    }
