 ...
 *
 * POST also accepts an optional `email`/`recipients` pair — the Reports →
 * Overview "Create Report" modal's own "Email this report after
 * generation" checkbox — sent via the same From-header-from-settings +
 * `wp_mail()` "ready" notice shape Controllers\Settings::send_test_report()
 * and vulopilot-pro's AdvancedReports\ScheduledReportRunner::maybe_email_report()
 * both already use, just to caller-supplied recipients instead of a fixed
 * notification address/schedule config. Deliberately free, not gated
 * behind the AdvancedReports module — unlike scheduled/recurring delivery
 * (still real Pro business logic, see that module's own docblock), emailing
 * the one report a user just generated by hand isn't a recurring-automation
 * feature.
 *
...
class Reports extends \WP_REST_Controller {
    ...
    public function create_item( $request ) {
        ...
        $email_result = $this->maybe_email_report( $request, $report );
    ...
    /**
     * "Email this report after generation" — the Create Report modal's
     * optional recipients field. A no-op (returns nulls, not false) when
     * `email` wasn't requested at all, so the frontend's success state can
     * tell "not sent" apart from "sending failed"; a no-op when the report
     * itself isn't `ready` too, since there's nothing to notify about yet.
     * Reuses the exact From-header + wp_mail() "ready" notice shape
     * Controllers\Settings::send_test_report() already establishes — same
     * body copy, just to these caller-supplied recipients instead of the
     * fixed notification_email.
     *
     * @param \WP_REST_Request          $request Full details about the request.
     * @param array<string, mixed>|null $report  The just-generated report row, or null.
     * @return array{email_requested: bool, email_sent: bool|null, email_recipients: string[]}
     */
    private function maybe_email_report( $request, $report ) {
        $email_requested = (bool) $request->get_param( 'email' );
        ...
        $raw_recipients = (string) $request->get_param( 'recipients' );
        $recipients     = array_values(
            array_filter(
                array_map(
                    ...
                    explode( ',', $raw_recipients )
                )
            )
        );

        if ( empty( $recipients ) || ! $report || 'ready' !== ( $report['status'] ?? '' ) ) {
            return array(
        ...
        $sent = wp_mail(
            ...
            __( 'Your VuloPilot report has been generated. Sign in to your dashboard\'s Reports page to view and download it.', 'vulopilot' ),
            $headers
        );
