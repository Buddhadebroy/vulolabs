 ...
 * other free AI surface uses when no AI provider is configured
...
class Copilot extends \WP_REST_Controller {
     ...
     * provider that will actually answer this turn reads one. No adapter
     * does today — see ProviderRegistry::supports_vision()'s own docblock.
     */
     ...
     * a configured AI provider (BYOK or a connected VuloCloud account) the
    ...
    private function resolve_first_image_attachment( array $raw_attachments ): ?array {
        foreach ( array_slice( $raw_attachments, 0, self::MAX_ATTACHMENTS ) as $attachment ) {
            ...
            return array(
                ...
                'payload'       => array(
                    'mime_type' => $mime,
                    'data'      => base64_encode( $contents ), // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode -- encoding real binary image bytes for AIRequest::get_image() (currently never read — ProviderRegistry::supports_vision() is always false, see its own docblock), not obfuscating anything.
                ),
            );
        }
