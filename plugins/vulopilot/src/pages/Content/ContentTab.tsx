 ...
 * SeoTab.tsx's own SEO_SECTIONS grouping). Same cross-category
 * `scannerIds`-prop mechanism SeoTab.tsx's own docblock already documents
 * for its own mixed-category scanner list — no `category` prop passed
 * below.
 */
 ...
 * — same "genuinely gates scanning" posture SeoTab.tsx's own
 ...
 * pattern GeoTab.tsx's own GeoScoreCard slot already uses.
 */
 ...
 * GeoVisibilitySummary's own slot on GeoTab.tsx.
 */
...
/**
 * "Content" tab of "Create Content" — depth, readability, and structure
 * across pages and posts. Body extracted verbatim from the former
 * standalone Content.tsx page (same "extract body, drop the header" move
 * every other folded-in tab this session already made) — its own
 * NavigatorHeaderComponent now lives once on Content.tsx's shared
 * tab-shell header.
 */
const ContentTab = () => {
	return (
