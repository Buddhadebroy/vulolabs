 ...
 * GEO.tsx's/AEO.tsx's own SCANNER_TO_SECTION documents.
 */
 ...
 * GEO.tsx and AEO.tsx already established for their own shared scanners).
 * Trust & Credibility, Multimedia & Visual Aids, and Terminology & Tone
 * have no scanner anywhere in this codebase, Free or Pro — checking any of
 * them (an expert-quote citation, a tone shift, a missing diagram) needs
 * real judgment, not pattern-matching, and this codebase's own established
 * posture is that AI-judged checks are always a Pro-gated engine
 * (GeoAnalysis\GeoAnalyzer, ContentIntelligence\ContentAnalyzer's own
 * `topic_authority` dimension), never a Free ScannerInterface implementer.
 * `scannerIds: []` renders an honest "not built yet" notice for those
 * three, same posture this page's own prior placeholder already took,
 * rather than a findings table that would sit permanently and misleadingly
 * empty.
 */
 ...
 * AI-judged signal that overlaps this page's own "Depth & Completeness"
 * category.
 */
...
/**
 * AI Content — the content-quality signals AI assistants weigh when
 * deciding what to summarize and cite: depth, trust, visuals, scannable
 * formatting, and terminology. Same header + findings-table shape every
 * other category page (GEO, AEO, SEO, Content) already uses.
 */
const AIContent = () => {
	...
	<>
		<NavigatorHeaderComponent
			...
			headerDescription={__(
				'The content signals AI assistants look for — depth, trust, visuals, formatting, and tone.',
				'vulopilot'
			)}
