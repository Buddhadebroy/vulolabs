...
| `PageSpeedInsightsFetcher` | `classes/Performance/PageSpeedInsightsFetcher.php` | Real Mobile/Desktop performance scores for "Performance" Overview's PerformanceScoreCard.tsx, via Google's real PageSpeed Insights API - only when the site owner has supplied their own `psi_api_key` (Settings → Scanning → Performa |
