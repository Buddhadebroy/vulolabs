...
it's the existing GEO/AEO feature set (`GeoAnalysis\GeoAnalyzer`, the 9+1 free
...
| AI Optimization (Pro) | The 6 existing GEO AI actions (`GenerateFaqAction`, `GenerateSummaryBlockAction`, `generate-author-bio`, `create-trust-page`, `soften-unsourced-claims`, `split-long-paragraphs`, `fix-heading-hierarchy`, `normalize-entity-naming`) |
...
(`email_on_geo_score_drop`/`geo_drop_threshold`, the same two settings
...
- `vulopilot_geo_visibility_trend` / `vulopilot_geo_competitor_visibility` (React filters, `@wordpress/hooks`) — same "register a source, don't modify the host" slot pattern `vulopilot_geo_score_card`/`vulopilot_geo_visibility_summary` already use on the GEO page.
