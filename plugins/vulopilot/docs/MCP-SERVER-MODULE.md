...
|---|---|---|
| `AiCopilot\ActionRunner::propose()` | Validates input → builds an AI prompt → calls the provider → validates the output → persists a `pending_approval` row in `vulopilot_ai_action_runs` and returns its preview. Never applies anything itself. | The entire mechanism behind "Approval-based execution only" - every Content/SEO/Visibility/Commerce tool calls this and nothing else. |
